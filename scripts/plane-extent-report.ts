...
function coverage(p: Plane): { occupied: number; dense: number } {
  const G = 40;
  const du = p.umax - p.umin,
    dv = p.vmax - p.vmin;
  const cnt = new Int32Array(G * G);
  let inside = 0;
  for (let i = 0; i < ply.positions.length; i += 3) {
    const x = ply.positions[i],
      y0 = ply.positions[i + 1],
      z0 = ply.positions[i + 2];
    // apply the same world rotation
    const y = c * y0 - s * z0,
      z = s * y0 + c * z0;
    const dx = x - p.centroid[0],
      dy = y - p.centroid[1],
      dz = z - p.centroid[2];
...
console.log(
  `${geom.planes.length} planes · radius ${radius.toFixed(2)} · orientation ${ORIENTS[det.index].name}\n`,
);
...
console.log(
  '(coverage = share of the DRAWN rectangle holding any point; dense = holding real density)',
);
