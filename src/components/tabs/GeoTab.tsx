...
import { CLS_CSS, DRIFT_BANDS } from '@/core/geometry/extract';
import {
  SEVERITY_CSS,
  buildVerdict,
  severityOf,
...
export default function GeoTab({
  ...
  const verdict = buildVerdict(g.planes);
  ...
  return (
    <>
      <div className="verdict" style={{ borderLeftColor: SEVERITY_CSS[verdict.level] }}>
        <div className="v-level" style={{ color: SEVERITY_CSS[verdict.level] }}>
          {LEVEL_WORD[verdict.level]}
        </div>
        <div className="v-head">{verdict.headline}</div>
        <div className="v-detail">{verdict.detail}</div>
      </div>

      {verdict.structural.length > 0 && (
        <>
          <div className="ghead">WALLS</div>
          {verdict.structural.map((p) => (
            <button key={p.label} className="wline" onClick={() => pick(p)}>
              <i style={{ background: SEVERITY_CSS[severityOf(p.drift ?? 0)] }} />
              <span className="lbl">{p.label}</span>
              <span className="say">{wallPhrase(p)}</span>
              <span className="num">{((p.tilt * 180) / Math.PI).toFixed(1)}°</span>
            </button>
          ))}
        </>
      )}

      {verdict.fragments.length > 0 && (
        <div className="gnote">
          {verdict.fragments.length} more wall surface
          {verdict.fragments.length === 1 ? ' was' : 's were'} found (
          {verdict.fragments.map((p) => p.label).join(', ')}) but came out too small or too patchy
          to judge, so they do not affect the reading above.
        </div>
      )}

      {rest.length > 0 && (
        <>
          <div className="ghead">FLOORS &amp; SLOPES</div>
          {rest.slice(0, 4).map((p) => (
            <button key={p.label} className="wline" onClick={() => pick(p)}>
              <i style={{ background: CLS_CSS[p.cls] }} />
              <span className="lbl">{p.label}</span>
              <span className="say">{surfacePhrase(p)}</span>
              <span className="num">{((p.tilt * 180) / Math.PI).toFixed(1)}°</span>
            </button>
          ))}
        </>
      ...
      <div className="ghead">DEBRIS</div>
      <div className="vfact">
        {g.debris.clusters.length === 0 ? (
          'None above the ground.'
        ) : (
          <>
            <b>{fmtNum(mVol(g.debris.totalVolume, mpu))} m³</b> across {g.debris.clusters.length}{' '}
            pile{g.debris.clusters.length === 1 ? '' : 's'}
            {mpu === 1 ? ' — assuming 1 scan unit is 1 metre' : ''}.
          </>
        )}
      </div>
      ...
      <details className="gnote-details">
        ...
        {!g.debris.clusters.length && (
          <div className="gempty">No debris above the ground plane.</div>
        )}
        ...
        <ul className="lim">
          <li>
            Not a mesh. Planes, angles and volumes are the measurable facts; a surface adds nothing
            to an assessment and hides the residual.
          </li>
          <li>
            A splat captures <b>exterior surfaces</b>. Debris volume is a column measure that
            assumes each pile is solid down to the ground plane — overhangs and interior voids are
            invisible.
          </li>
          <li>
